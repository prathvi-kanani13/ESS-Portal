...
export const StatsCard: React.FC = () => {
  ...
  const handleCardClick = (item: Stat) => {
    if (item.title === "Attendance") setAttendanceOpen(true);
    if (item.title === "WFH/Approvals") setWfhOpen(true);
    if (item.title === "Leaves") setOpenLeave(true);
  };

  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4 mb-1">
      {statsData.map((item, idx) => (
        ...
        >
          <div className="flex flex-col justify-start gap-1 sm:gap-2">
            <p className="text-xs sm:text-sm font-medium">{item.title}</p>
            <p className="text-xl sm:text-2xl font-bold">{item.value}</p>
            <div className="flex items-center text-[9px] sm:text-xs opacity-90 gap-1">
          ...
          <div className="w-16 h-16 sm:w-20 sm:h-20 relative">
            ...
            <span className="absolute inset-0 flex items-center justify-center text-[9px] sm:text-xs font-bold">
              {item.progress}%
            </span>
          </div>
        </Card>
